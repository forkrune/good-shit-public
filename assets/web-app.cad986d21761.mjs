...
import { renderUiIcon, setUiIcon } from './src-core-ui-icons.eb64a1f4d6b3.mjs';

const pageConfigNode = document.querySelector('#page-config');
const pageConfig = pageConfigNode ? JSON.parse(pageConfigNode.textContent) : {};
const ui = (name) => renderUiIcon(pageConfig.uiIconSpriteUrl, name);
...
function updateQuickActions(element, record) {
  ...
  if (favourite) {
    ...
    setUiIcon(favourite, pageConfig.uiIconSpriteUrl, record.favourite.value ? 'heart-filled' : 'heart');
...
function updateDetailPanel(element, record) {
  ...
  if (favourite) {
    favourite.setAttribute('aria-pressed', String(record.favourite.value));
    favourite.innerHTML = `${ui(record.favourite.value ? 'heart-filled' : 'heart')} ${record.favourite.value ? 'Favourited' : 'Favourite'}`;
  }
  if (visited) {
    visited.setAttribute('aria-pressed', String(record.visited.value));
    visited.innerHTML = `${ui('check')} ${record.visited.value ? 'Visited' : 'Mark visited'}`;
  }
