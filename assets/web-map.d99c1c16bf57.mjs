...
import { documentMatchesFilters, querySearchIndex } from './src-core-search.ce34e4149197.mjs';
import { tierRank } from './src-core-constants.2abfb1694768.mjs';
import { escapeHtml, joinUrl, renderEntityGrid } from './web-client-render.0c3bd14f5671.mjs';
...
function installSourcesAndLayers() {
  ...
  map.addLayer({
    ...
    paint: {
      ...
      'circle-radius': ['interpolate', ['linear'], ['zoom'], 3, 5, 10, 8, 16, 11],
      'circle-stroke-color': '#ffffff',
...
function popupHtml(document) {
  return `<div class="stack-sm"><span class="tier-chip"><strong>${escapeHtml(document.tier)}</strong> curated</span><strong>${escapeHtml(document.name)}</strong><span class="muted">${escapeHtml([document.locality, document.country].filter(Boolean).join(' · '))}</span><a href="${escapeHtml(joinUrl(config.basePath, `places/${document.slug}/`))}">Open details →</a></div>`;
}
...
async function locateUser() {
  ...
  navigator.geolocation.getCurrentPosition(async (position) => {
    ...
    button.textContent = 'Near me active';
