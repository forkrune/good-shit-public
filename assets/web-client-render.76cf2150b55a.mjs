...
const ui = (name, className) => renderUiIcon(pageConfig.uiIconSpriteUrl, name, className);
...
function snapshot(document) {
  return { name: document.name, slug: document.slug, countryCode: document.countryCode, tier: document.tier, status: 'published' };
}

function locationLabel(document) {
  return [document.locality, document.region, document.country].filter(Boolean).filter((value, index, array) => index === 0 || value !== array[index - 1]).join(' · ');
}
  ...
  return `<article class="entity-card tier-${escapeHtml(String(document.tier).toLowerCase())}${selected}" data-entity-id="${escapeHtml(document.id)}" data-entity-snapshot="${escapeHtml(safeJson(snapshot(document)))}">
    <a class="card-main" href="${escapeHtml(href)}">
      ...
      <div class="card-topline">
        <span class="type-chip">${renderPlaceIcon(pageConfig.mapIcons, document.iconKey ?? document.entityType)}${escapeHtml(String(document.entityType).replaceAll('-', ' '))}</span>
    ...
    <div class="card-actions" aria-label="Personal actions for ${escapeHtml(document.name)}">
      <button type="button" class="icon-button" data-personal-action="favourite" aria-pressed="false" title="Favourite">${ui('heart')}<span class="sr-only">Favourite</span></button>
      <button type="button" class="icon-button" data-personal-action="visited" aria-pressed="false" title="Visited">${ui('check')}<span class="sr-only">Visited</span></button>
      <a class="icon-button" data-map-select="${escapeHtml(document.id)}" href="${escapeHtml(mapHref)}" title="Show on map">${ui('map')}<span class="sr-only">Show on map</span></a>
    </div>
  </article>`;
}

export function renderEntityGrid(documents, options = {}) {
  if (!documents.length) return `<div class="empty-state">${ui('empty', 'ui-icon empty-icon')}<h2>${escapeHtml(options.emptyTitle ?? 'No curated places match this view.')}</h2><p>${escapeHtml(options.emptyText ?? 'This does not mean nothing exists here; it means nothing currently published meets or is known to meet the catalogue bar.')}</p></div>`;
