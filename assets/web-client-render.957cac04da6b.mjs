...
function iconForType(type) {
  return { food: '◉', entertainment: '▶', 'natural-poi': '⌁', 'hiking-route': '↟', architecture: '◇', shop: '▣', experience: '✦' }[type] ?? '●';
}
...
export function renderEntityGrid(documents, options = {}) {
  if (!documents.length) return `<div class="empty-state"><span aria-hidden="true">◇</span><h2>${escapeHtml(options.emptyTitle ?? 'No curated places match this view.')}</h2><p>${escapeHtml(options.emptyText ?? 'This does not mean nothing exists here; it means nothing currently published meets or is known to meet the catalogue bar.')}</p></div>`;
